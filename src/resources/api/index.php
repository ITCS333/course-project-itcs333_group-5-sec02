...
function getAllResources($db) {
    ...
    $sort = isset($_GET['sort']) && in_array($_GET['sort'], $validSortFields) ? $_GET['sort'] : 'created_at';
...
function getResourceById($db, $resourceId) {
    ...
    if ($resource) {
            sendResponse(true, $resource);
        } else {
            sendResponse(false, ['error' => 'Resource not found'], 404);
        }
    } catch (PDOException $e) {
        sendResponse(false, ['error' => $e->getMessage()]);
    }
...
function createResource($db, $data) {
    ...
    if (empty($data['title']) || empty($data['link'])) {
        sendResponse(false, ['error' => 'Title and link are required'], 400);
        return;
    }
    ...
    if (!filter_var($link, FILTER_VALIDATE_URL)) {
        sendResponse(false, ['error' => 'Invalid URL format'], 400);
        return;
    }
    ...
    // If no, return error response with 500 status
     if ($success) {
            ...
            sendResponse(true, ['id' => $newId], 201);
        } else {
            sendResponse(false, ['error' => 'Failed to create resource'], 500);
        }
...
function updateResource($db, $data) {
    ...
    // If not, return error response with 400 status
    if (empty($data['id'])) {
        sendResponse(false, ['error' => 'Resource ID is required'], 400);
        return;
    }
    ...
    // TODO: If no fields to update, return error response with 400 status
     if (empty($fields)) {
            sendResponse(false, ['error' => 'No fields provided for update'], 400);
            return;
        }
    ...
    // If no, return error response with 500 status
      if ($success) {
            sendResponse(true, ['message' => 'Resource updated successfully'], 200);
        } else {
            sendResponse(false, ['error' => 'Failed to update resource'], 500);
        }
...
function deleteResource($db, $resourceId) {
    ...
    try {
        ...
        $deleteComments = $db->prepare("DELETE FROM comments WHERE resource_id = ?");
        // TODO: Bind resource_id and execute
        $deleteComments->bindValue(1, $resourceId, PDO::PARAM_INT);
       ...
       $deleteResource = $db->prepare("DELETE FROM resources WHERE id = ?");
        // TODO: Bind resource_id and execute
        $deleteResource->bindValue(1, $resourceId, PDO::PARAM_INT);
        $deleteResource->execute();
        // TODO: Commit the transaction
        // Use $db->commit()
         $db->commit();
        // TODO: Return success response with 200 status
         sendResponse(true, ['message' => 'Resource deleted successfully'], 200);
     } catch (Exception $e) {
...
function createComment($db, $data) {
    ...
    if (empty($data['resource_id']) || empty($data['author']) || empty($data['text'])) {
        sendResponse(false, ['error' => 'resource_id, author, and text are required'], 400);
        return;
    }
    ...
    if ($success) {
            ...
            sendResponse(false, ['error' => 'Failed to create comment'], 500);
        }
...
function deleteComment($db, $commentId) {
    ...
    if (empty($commentId) || !is_numeric($commentId)) {
        sendResponse(false, ['error' => 'Invalid comment ID'], 400);
        return;
    }
    ...
    // If not found, return error response with 404 status
      $checkStmt = $db->prepare("SELECT id FROM comments WHERE id = ?");
        ...
        if (!$exists) {
            sendResponse(false, ['error' => 'Comment not found'], 404);
            return;
        }
    ...
    // TODO: Bind the comment_id parameter
     $deleteStmt->bindValue(1, $commentId, PDO::PARAM_INT);
    // TODO: Execute the query
    ...
    // If no, return error response with 500 status
     if ($success) {
            sendResponse(true, ['message' => 'Comment deleted successfully'], 200);
        } else {
            sendResponse(false, ['error' => 'Failed to delete comment'], 500);
        }
...
try {
    ...
    if ($method === 'GET') {
        ...
        if ($action === 'comments') {
            ...
            $resourceId = intval($_GET['resource_id']);
